...
mzml=~/data/TCGA_CC/TCGA-AA-3518
...
for file in *.mzML; do
	pin=$file.pin.tsv
	pout=$file.pout.tsv
	mzml_file=`basename $file`
	msgf2pin -F $fasta_target,$fasta_decoy --min-length 7 -o $pin $mzml/target/$mzml_file.mzid $mzml/decoy/$mzml_file.mzid 
	percolator $pin -f $fasta_target > $pout
done
